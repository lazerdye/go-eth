// Source: https://raw.githubusercontent.com/0xProject/0x-mesh/ee90a94d646da9acb675c2ae2785128695b12678/zeroex/asset_data_decoder.go
...
func (a *AssetDataDecoder) Decode(assetData []byte) (interface{}, error) {
	if len(assetData) < 4 {
	...
	info, ok := a.idToAssetDataInfo[idHex]
	if !ok {
		return nil, fmt.Errorf("Unrecognized assetData with prefix: %s", idHex)
	}
	...
	return info.abi.Methods[info.name].Inputs.Unpack(assetData[4:])
}
